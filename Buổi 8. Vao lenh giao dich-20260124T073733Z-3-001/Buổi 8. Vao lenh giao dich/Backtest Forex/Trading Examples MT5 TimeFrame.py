...
def loaddataMT5(symbol: str = "VNM", days: int = 5, timeframe: str = mt5.TIMEFRAME_D1) -> pd.DataFrame:
    ...
    try:
        ...
        data = CommonMT5.loaddataMT5_FromTo(symbol, from_date, to_date, timeframe=timeframe)
        ...
        print(f"Đã lấy thành công {days} ngày dữ liệu cho {symbol}")
        
        return data
        
    except Exception as e:
        print(f"Lỗi khi lấy dữ liệu từ {symbol} từ {from_date} đến {to_date}: {e}")
...
def simple_ma_strategy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Signal_Reason'] = df.apply(
        lambda row: f"SMA5 ({row['SMA_5']:.5f}) > SMA20 ({row['SMA_20']:.5f})" if row['Buy_Signal']
        else f"SMA5 ({row['SMA_5']:.5f}) < SMA20 ({row['SMA_20']:.5f})" if row['Sell_Signal']
...
class BacktestEngine:
    ...
    def __init__(self, initial_balance: float = 10000000):
        ...
        logger.info(f"Backtest engine initialized with balance: {initial_balance:,.0f} USD")
    ...
    def execute_signal(self, signal: TradingSignal) -> Optional[Trade]:
        ...
        if signal.signal == "BUY":  # Vẫn giữ string "BUY"/"SELL" vì đây là TradingSignal object
            ...
            max_quantity = int(self.balance / current_price)
            if max_quantity <= 0:
                logger.warning(f"Insufficient balance to buy {symbol}")
                return None
            ...
            self.balance -= cost
            self.portfolio[symbol] = self.portfolio.get(symbol, 0) + max_quantity
            self.current_positions[symbol] = trade
            
            logger.info(f"BUY {max_quantity} {symbol} @ {current_price:,.5f} VND")
            return trade
            ...
            logger.info(f"SELL {quantity} {symbol} @ {current_price:,.5f} VND, P&L: {current_trade.pnl:,.5f} VND ({current_trade.pnl_pct:.2f}%)")
    ...
    def run_backtest(self, df: pd.DataFrame, signals: List[TradingSignal]):
        ...
        for signal in signals:
            ...
            portfolio_value = sum(self.portfolio.get(s, 0) * signal.price for s in self.portfolio)
        ...
        for symbol, trade in self.current_positions.items():
            ...
            logger.info(f"Closed position: {symbol}, P&L: {trade.pnl:,.5f} VND ({trade.pnl_pct:.5f}%)")
...
def print_backtest_summary(results: Dict[str, float]):
    ...
    print("=" * 60)
    print(f"Số dư ban đầu:     {results['initial_balance']:>15,.0f} USD")
    print(f"Equity cuối:        {results['final_equity']:>15,.0f} USD")
    ...
    print(f"Lợi nhuận TB:       {results['avg_win']:>15,.0f} USD")
    print(f"Thua lỗ TB:         {results['avg_loss']:>15,.0f} USD")
...
def main():
    ...
    for i in range(20, len(display_df)):
        ...
        if display_df['Buy_Signal'].iloc[i]:  # True = có tín hiệu mua
            print(f"     {signal_count+1}. {display_df.index[i].strftime('%Y-%m-%d')} - BUY @ {display_df['Close'].iloc[i]:,.5f} ")
            ...
            print(f"     {signal_count+1}. {display_df.index[i].strftime('%Y-%m-%d')} - SELL @ {display_df['Close'].iloc[i]:,.5f} ")
            print(f"        Lý do: {display_df['Signal_Reason'].iloc[i]}")
            print(f"        Độ tin cậy: {display_df['Signal_Confidence'].iloc[i]:.1%}")
            signal_count += 1
